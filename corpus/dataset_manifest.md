...
- **Diagnosis:** all 9/9 verified authorities are absent at k=500. Frozen facts-only inputs range from 844--19,973 words (median 5,861), but the FTS5 query keeps only the first 32 terms, which are frequently procedural boilerplate. This is an evidence-backed lexical-mismatch limitation, not a test-set result.
- **Freeze decision:** no fix was attempted under the bounded one-fix rule because absence persisted for every dev case at k=500. Keep `week7-bm25-diverse-support-v1` unchanged (candidate k=100, first-32-term FTS5 OR query, five-source diverse selection) for Week 10's reproducibility freeze. See `artifacts/dev_retrieval_investigation_corrected.md` and `artifacts/dev_retrieval_probe_corrected.json`.
