...
- **Final regression and freeze decision:** a single paired check on six pre-specified real answer-key controls compared legacy and salient queries at k=100 and k=500 using the same full facts-only inputs and index. Salient terms caused no rank degradation and improved `1988_96` from absent to rank 13/selected. Adopt `week10-bm25-salient-terms-v1`: candidate k=100, `tfidf-segment-salient-terms-v1` query construction, and five-source diverse selection. The remaining recall limitations remain reportable, but the known first-32-term defect is no longer the frozen runtime path. See `artifacts/week9_final_freeze_regression.md`.
