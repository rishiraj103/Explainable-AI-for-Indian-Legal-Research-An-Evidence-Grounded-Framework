...
- **Finality and precision context:** the Week 11 n=1,503 E1/E2 outcome results and n=30 E3/E4 evidence results are final for this evaluation round. A later ten-case answer-key extension will be separate and will not overwrite these records. Authority-consistent precision is 11/135 = 0.081481 over selected/displayed evidence items. Because the renderer displays 4.5 citations per case on average while each query has one reference authority, its fixed-cardinality ceiling is 1/4.5 = 0.222222; observed precision is 36.7% of that ceiling. See `artifacts/week11_initial_results.md` and `config/week11_evaluation_round.json`.

## Week 12 frozen-result error analysis

- **Scope:** analysis is derived only from the finalized Week 11 artifacts; it does not rerun a model, modify retrieval, change the answer key, or tune on test outcomes.
- **Exhaustive retrieval buckets:** 11/30 expected authorities were retrieved and selected; `1980_133` was retrieved but not selected (first matching chunk rank 48); and 18/30 were absent at k=100. This makes retrieval coverage—not provenance, grounding, or date filtering—the binding limitation.
- **Integrity and relevance interpretation:** all 135 displayed citations remained provenance-valid and temporally eligible with no unsupported-claim detections. Of these, 124 differ from the single reference authority in the key; that is an answer-key-consistency measure, not a determination that the alternative citation is substantively irrelevant without human relevance annotation.
- **Baseline-analysis limitation:** the frozen E1/E2 artifacts retain aggregate metrics and confusion matrices but not per-case prediction vectors, so no post-hoc model-disagreement example is claimed. See `artifacts/week12_error_analysis.md`.
