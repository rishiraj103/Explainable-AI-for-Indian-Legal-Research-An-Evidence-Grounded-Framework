...
| --- | --- | --- | --- |
| Dataset version and split: ILDC Single and eCourts | **PASS** | `config/reproducibility_freeze.json` -> `datasets_and_splits`; `config/datasets.json`; `artifacts/ecourts_corpus_identity.json`; `corpus/dataset_manifest.md` | ILDC source revision, fixed split rows, and all three Parquet hashes are explicit. The stale count is corrected to `2,343,435`. The final cleaned eCourts JSONL corpus is identified by a 71-file, 2,343,435-record aggregate SHA-256 (`f0229b…f000f`) and linked to the BM25 index/cleaning-record hashes. This identifies the local final corpus bytes, not an immutable upstream S3 snapshot or PostgreSQL volume dump. |
...
| Model checkpoints: E1 reconstruction and E2 checkpoint-6318 | **PASS** | `config/reproducibility_freeze.json` -> `experiments.E1.reconstructed_model` and `experiments.E2_corrected.checkpoint_6318`; `corpus/dataset_manifest.md` -> Prediction-recovery correction | The freeze now records the E1 joblib hash `60f407…efec8` and E2 safetensors hash `924a5b…c773`, plus checkpoint config/trainer-state hashes. E2 weights remain gitignored by design because of size; their tracked hashes provide an explicit verification contract before inference/replay. |
...
| Software dependencies, PostgreSQL, and Docker images | **PASS** | `config/reproducibility_freeze.json` -> `environment`, `environment.transformers_runtime_roles`, `environment.postgresql`, and `environment.container_image_policy`; `compose.yaml`; `docker/Dockerfile.ocr`; `docker/e2.Dockerfile` | The version distinction is now explicit by role: E2 training used Docker-pinned Transformers `4.46.3`, while the later host replay used `5.15.0` and reproduced the checkpoint exactly. PostgreSQL `16.15`, the observed Postgres image digest, and Dockerfile/compose hashes are tracked. The remaining mutable base-image tags are explicitly accepted for this local research deployment, with their last frozen build-definition revision/date recorded; they are not misrepresented as production-grade immutable infrastructure. |

Documentation result after remediation: **14 PASS, 0 FAIL**. The three original findings are closed proportionately: the final cleaned corpus now has a deterministic local identity, checkpoint hashes are frozen even where weights remain deliberately local-only, and the environment/container roles and accepted mutable-tag boundary are explicit.

Expanded freeze-integrity check: **34/34 path/SHA-256 pairs passed** against the current local files, including the newly recorded E1 model, E2 checkpoint files, eCourts corpus identity, Dockerfiles, and Compose definition. There were zero missing files and zero hash mismatches.
